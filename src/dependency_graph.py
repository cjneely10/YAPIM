...
class DependencyGraph:
    ...
    def _build_dependency_graph(self, tasks: List[BaseTask]):
        ...
        for task_list in tasks:
            task_node = Node(name=task_list.task_name, scope="", dependency_input="root", id_mapping=None)
            ...
            for dependency in task_list.depends:
                if not isinstance(dependency, DependencyInput):
                    raise DependencyGraph.ERR
                self._add_requirements_within_dependencies(self.graph, Node(name=dependency.name,
                                                                            scope=task_list.task_name,
    ...
    def _add_requirements_within_dependencies(self, graph: nx.DiGraph, node: Node, task_node: Node, attr: str,
        ...
        for requirement in getattr(self.idx[node.name], attr):
            if attr == "depends":
                ...
                new_node = Node(
                    name=requirement, scope=scope, dependency_input="root", id_mapping=None
                )
            graph.add_edge(new_node, task_node)
            self._add_requirements_within_dependencies(graph, new_node, task_node, attr, scope)

    def _get_dependencies_at_level(self, task: BaseTask) -> \
            List[Tuple[Type[BaseTask], str, str, List[Tuple[str, str]]]]:
        ...
        """
        graph = nx.DiGraph()
        task_node = Node(name=task.task_name, scope="", dependency_input="root", id_mapping=None)
        graph.add_node(task_node)
        for dependency in task.depends:
            if not isinstance(dependency, DependencyInput):
                raise DependencyGraph.ERR
            graph.add_edge(
                Node(name=dependency.name, scope=task.task_name, dependency_input=dependency.input,
                     id_mapping=dependency.id_mapping), task_node
            )
            self._add_requirements_within_dependencies(
                graph, Node(name=dependency.name, scope=task.task_name, dependency_input=dependency.input,
                            id_mapping=dependency.id_mapping), task_node, "depends", task.task_name
            )
    ...
    @property
    def sorted_tasks(self) -> List[Tuple[BaseTask, str, str, List[Tuple[str, str]]]]:
