...
class TestConfigManager(unittest.TestCase):
    cfg = ConfigManager(Path(__file__).parent.joinpath("config_files").joinpath("valid-config.yaml"))
    ...
    def test_bad_data(self):
        with self.assertRaises(MissingDataError):
            ConfigManager(Path(__file__).parent.joinpath("config_files").joinpath("bad_data-config.yaml"))

    def test_invalid_program(self):
        with self.assertRaises(InvalidPathError):
            ConfigManager(Path(__file__).parent.joinpath("config_files").joinpath("bad_program-config.yaml"))

    def test_invalid_dependencies_section(self):
        with self.assertRaises(MissingDataError):
            ConfigManager(Path(__file__).parent.joinpath("config_files").joinpath("invalid_dependency_section-config.yaml"))
    ...
    def test_bad_resources(self):
        with self.assertRaises(InvalidResourcesError):
            ConfigManager(Path("config_files").joinpath("bad_resources-config.yaml"))
